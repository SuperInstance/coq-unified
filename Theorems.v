...
Section Deadband.
  ...
  Hypothesis epsilon_0_pos : (0 < epsilon_0)%R.
  Hypothesis lambda_pos    : (0 < lambda)%R.
  ...
  Lemma exp_le_1 : forall dt, (0 <= dt)%R -> (exp (- lambda * dt) <= 1)%R.
  Proof.
    intros dt Hdt.
    assert (H1 : (- lambda * dt <= 0)%R).
    { nra. }
...
Section Metronome.
  ...
  Lemma diameter_nonincreasing :
    forall alpha f,
      (0 <= alpha <= 1)%R ->
      (fleet_diameter (tick_step alpha f) <= fleet_diameter f)%R.
  Proof.
